...
class GoBoard: #定义棋盘
    def __init__(self,width=19,height=19,):
        ...
        self.width=width
        self.height=height
        self.stones={} #dictionary,记录棋盘上的落子位的子的情况，key：(0,0)，value：StoneString
    ...
    def envUpdate(self,player,stone): #在棋盘上放子,stone就是move
        if stone==(-5,-5): #pass
        ...
        elif stone==(-10,-10): #resign
            self.move_records.append((player,stone)) #记录这步操作
        else:
            #先在棋盘的stone点位上形成这颗子string，根据string的定义，要做到这一点，先要获取stone周围的情况
            uldr=self.getStoneNeighbours(stone) #stone=(0,0)
            ...
            for i in uldr: #开始一一甄别周围子的情况,i=(0,0)
                if not self.isOnBoard(i): #超出棋盘大小就跳过
                    continue
                string=self.stones.get(i)
                #string就三种情况，空位，黑子，白子
                if string==None:
                    liberties.append(i)
                elif string.becolgings==player:
                    if string not in same_strings:
                        same_strings.append(string)
                else:
                    if string not in opposite_strings:
                        opposite_strings.append(string)
            new_string=StoneString(player,[stone],liberties) #先把当前落子看作独立的stoneString，相当于落子了
            #落子后要更新棋盘的zobrist值
            self.updateZobrist(player,stone)
            for i in same_strings: #把相邻的己方的子连成一个新的StoneString
                new_string=StoneString.merge(new_string,i)
            for i in new_string.stones: #更新棋盘的相关点位的新string
                self.stones[i]=new_string
            for i in opposite_strings: #检查相邻对方的子，减掉他们的气，如果气绝就要提走
                i.liberties.discard(stone)
                #如果提子，还得更新棋盘的zobrist值
                if len(i.liberties)==0: #提子
                    for j in i.stones: #更新棋盘的相关点位已被新的string占领
                        #删掉前还得为己方计算获得的新增的气
                        for k in self.getStoneNeighbours(j):
    ...
    def getStoneNeighbours(self,stone): #获取落子点上下左右的点位
        return [ #按上、左、下、右逆时针的顺序
            (stone[0]-1,stone[1]),
            (stone[0],stone[1]-1),
            (stone[0]+1,stone[1]),
            (stone[0],stone[1]+1)
        ]
    def isOnBoard(self,stone): #判断点位是否在棋盘内
        return 0<=stone[0]<self.height and 0<=stone[1]<self.width

    def updateZobrist(self,player,stone):
        self.zobrist^=HASH_CODE.get((stone, player))

    def evaluate(self,player,stone): #预评估下棋后的棋形
        ...
        zobrist^=HASH_CODE.get((stone, player))
        for i in same_strings: #把相邻的己方的子连成一个新的StoneString
            new_string=StoneString.merge(new_string,i)
        for i in new_string.stones: #更新棋盘的相关点位的新string
            stones[i]=new_string
        for i in opposite_strings: #检查相邻对方的子，减掉他们的气，如果气绝就要提走
            ...
            if len(i.liberties)==0: #提子
                for j in i.stones: #更新棋盘的相关点位已被新的string占领
                    #删掉前还得为己方计算获得的新增的气
                    for k in self.getStoneNeighbours(j):
                        if stones.get(k) is not None and stones.get(k).becolgings==player:
                            stones.get(k).liberties.add(j)
                    stones.pop(j)
                    zobrist^=HASH_CODE.get((j, player.other()))
        return len(stones.get(stone).liberties),zobrist

    def printBoard(self): #打印出棋盘
    ...
    def toNormalBoard(self): #把board转换成numpy数组，现实中可以理解的样子
        board_array=np.zeros((self.height,self.width)) #空位是0
        for i in self.stones:
            if self.stones[i].becolgings==Player.black: #黑棋为1
                board_array[i[0],i[1]]=1
            elif self.stones[i].becolgings==Player.white: #白棋为-1
                board_array[i[0],i[1]]=-1
            else:
                pass
        return board_array
